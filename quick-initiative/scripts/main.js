...
const QiBanner = {
    ...
    _applyBackdropBg() {
        if (!this._backdrop) return;
        let img = '';
        try { img = game.settings.get(QI_ID, 'bannerBg') ?? ''; } catch(e) {}
        if (img) {
            // Mantém escurecimento semi-transparente por cima da imagem
            this._backdrop.style.background =
                `linear-gradient(rgba(0,0,0,.52),rgba(0,0,0,.52)), url('${img}') center/cover no-repeat`;
        } else {
            this._backdrop.style.background = ''; // usa o CSS padrão (rgba(0,0,0,.6))
        }
    },
    ...
    _renderTrack(state, direction = null) {
        ...
        this._track.innerHTML = `
            <div class="qt-inner">
                ...
                </div>` : '<div class="qt-nav-spacer"></div>'}
                <div class="qt-carousel" id="qt-cards-list">
                    ${cards}
                </div>
                ${isGM ? `<div class="qt-right-col">
        ...
        if (isGM) {
            /* ── Detecta em qual região da tela o tracker está e aplica dock-* ── */
            const updateDock = () => {
            ...
            /* ── Todos os botões lock sincronizados ── */
...
class QuickInitiative {
    static init()  {
        ...
        game.settings.register(QI_ID, 'bannerBg', {
            ...
            hint   : [
                ...
                'Tamanho atual do banner: largura 100% da tela × 360 px de altura.',
                '✅ Recomendado: 1920 × 360 px  |  Mínimo sugerido: 1280 × 360 px',
                'Imagens maiores (ex: 1920×1080) funcionam normalmente — serão cortadas ao centro.',
    ...
    static ready() {
        ...
        // Aplica cores imediatamente ao carregar (antes de qualquer combate)
...
class QuickInitiativeSetup extends Application {
    ...
    async _renderInner() {
        // Sistemas principais (exceto genérico)
        const mainSystems = Object.entries(QI_SYSTEMS).filter(([k]) => k !== 'generic');
        const allSystems  = [...mainSystems, ['generic', QI_SYSTEMS.generic]];
